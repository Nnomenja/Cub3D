...
/*   Updated: 2025/11/25 09:42:02 by nnomenja         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	fill_map_data(t_map_data *map_data, char *tok)
{
	...
	else if (str_n_cmp(tok, "G ", 2) == 0 \
	&& check_extension(tok, str_len(tok)))
		map_data->g = str_dup(tok);
	else if (str_n_cmp(tok, "GW ", 3) == 0 \
	&& check_extension(tok, str_len(tok)))
		map_data->gw = str_dup(tok);
	else
		return (0);
	return (1);
}
...
t_map_data	get_map_data(char *map)
{
	...
	v.map_data = (t_map_data){NULL, NULL, NULL, NULL, NULL, \
	NULL, NULL, NULL, NULL};
	v.i = 0;
